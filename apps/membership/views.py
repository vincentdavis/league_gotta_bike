...
from .forms import (
    MembershipInviteForm, MembershipRoleUpdateForm,
...
class MembershipInviteView(OrgMemberManagerRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f'{user.get_full_name()} has been added to {organization.name} as {dict(Membership.ROLE_CHOICES)[role]}.'
        )
...
class MembershipLeaveView(LoginRequiredMixin, View):
    ...
    def get(self, request, org_id):
        ...
        if membership.role == Membership.ORG_OWNER:
